...
public class ReportProcessor extends javax.swing.JInternalFrame {
    ...
    private void idTerakhir(){
        try{
            String sql = "select id_p from processor";
             pst = conn.prepareStatement(sql);
             rs = pst.executeQuery();
             
             while(rs.next()){
                 String dataku = rs.getString("id_p");
    ...
    private void kodeTerakhir(){
        ...
        try{
            String sql = "select kode_p from processor";
             pst = conn.prepareStatement(sql);
             ResultSet rs = pst.executeQuery();
             if(!rs.isBeforeFirst()){
                 String KodeTerbaru = "PR0001";
                 txtKode.setText(KodeTerbaru);
             }else{
                while(rs.next()){
                    String kodeku = rs.getString("kode_p");
                    String dapatAngka= kodeku.replaceAll("[^0-9]", "");
                    int kodeBaru = Integer.parseInt(dapatAngka)+1;
                    String KodeTerbaru = "PR000"+kodeBaru;
    ...
    private void UpdateTabel(){
        ...
        try{
            ...
            while(rs.next()){
                ...
                a[0] = rs.getString("id_p");
                a[1] = rs.getString("kode_p");
                a[2] = rs.getString("nama_p");
                a[3] = rs.getString("merek_p");
                a[4] = rs.getString("harga_p");
                a[5] = rs.getString("stock_p");
                tabelMb.addRow(a);
            }
            tabelMotherboard.setModel(tabelMb);
            tabelMotherboard.getColumnModel().getColumn(0).setPreferredWidth(20);
            tabelMotherboard.getColumnModel().getColumn(1).setPreferredWidth(40);
            tabelMotherboard.getColumnModel().getColumn(2).setPreferredWidth(350);
            tabelMotherboard.getColumnModel().getColumn(3).setPreferredWidth(100);
    ...
    private void tampilkan_merek(){
        try{
             String sql = "select nama_merek from merek where kat_p !=0";
             pst = conn.prepareStatement(sql);
             rs = pst.executeQuery();
    ...
    private void initComponents() {
        ...
        jLabel3.setText("= Master Data Processor = ");
    ...
    private void btnEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEditActionPerformed
        try{
            Connection conn = null;
            Statement stmt = null;
            String sql = "Update processor SET kode_p ='"+txtKode.getText()+"', nama_p ='"+txtNama.getText()+"', merek_p ='"+cmbMerek.getSelectedItem()+"',harga_p ='"+txtHarga.getText()+"', stock_p ='"+txtStock.getText()+"' WHERE id_p ='"+txtID.getText()+"'";
           ...
           cmbMerek.getSelectedItem().equals("=Pilih=")){
            ...
            if (String.valueOf(input).equals("0")){
                try{
                    Connection conn = null;
                    Statement stmt = null;
                    String sql = "DELETE from processor WHERE id_p='"+txtID.getText()+"'";
                    conn = simanu.KoneksiDB.simanuDb();
                    stmt = conn.createStatement();
                    stmt.executeUpdate(sql);
                    UpdateTabel();
        ...
        if(txtKode.getText().equals("") ||
           ...
           cmbMerek.getSelectedItem().equals("=Pilih=")){
            ...
            try{
                ...
                conn = simanu.KoneksiDB.simanuDb();
                stmt = conn.createStatement();
                stmt.executeUpdate("insert into processor(kode_p,nama_p,merek_p,harga_p,stock_p) values('"+kodeM+"','"+namaM+"','"+cmbM+"','"+hargaM+"','"+stockM+"')");
